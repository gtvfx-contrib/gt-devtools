...
Import-Module MessageBox -Force

$registryKey = 'HKCU:\Software\gt-devtools'

# ── PATH and PSModulePath setup ──────────────────────────────────────────────
    
$pathsToAdd = @('pwsh', 'cmd') | ForEach-Object { Join-Path $PSScriptRoot $_ }
...
if ($previousRoot -and $previousRoot -ne $PSScriptRoot) {
    foreach ($stale in @('pwsh', 'cmd') | ForEach-Object { Join-Path $previousRoot $_ }) {
...
if ($pathUpdated) {
    ...
    Show-MessageBox `
        -message "Setup complete.`n`nPATH has been updated — please restart your terminal for the changes to take effect." `
        -title 'gt-devtools Setup' 
} else {
    Write-Host "`nSetup complete. Context menu entries updated." -ForegroundColor Cyan
    Show-MessageBox `
        -message "Setup complete. Context menu entries have been updated.`n`nPATH was already configured — no terminal restart needed." `
        -title 'gt-devtools Setup'
}
