...
2. **Choose your authentication settings:**
   ...
   - **GHE hostname:** Enter `<Enterprise Host URL>`
   - **Preferred protocol for Git operations?** Choose `SSH`
   - **Upload your SSH public key to your GitHub account?** Choose `Yes` (it should find your SSH key for your github account)
...
```powershell
Import-Module "...\gt\devtools\pwsh\github\GitHubTeamManagement.psm1"
```

To reload the module after making changes:
```powershell
Import-Module "...\gt\devtools\pwsh\github\GitHubTeamManagement.psm1" -Force
```
...
**Parameters:**
- `Org` - Organization name (default: 'gtvfx-contrib')
...
$repos = Get-GitHubRepos -SkipPattern @("gt-ext-*", "*-archive")
...
Add-GitHubTeamToRepos -Team "envoy-internal-dev" -Permission "push"
...
**Example:**
```powershell
$team = Get-GitHubTeam -Team "envoy-internal-dev"
$team | Format-List
```
...
$content = @(
    ...
    "* @gtvfx-contrib/envoy-internal-dev"
)
Update-GitHubCodeowners -Content $content

# Update with skip patterns
Update-GitHubCodeowners -Content "* @gtvfx-contrib/team" -SkipPattern @("gt-ext-*", "*-archive")

# Update specific repos in root directory
Update-GitHubCodeowners -Content "* @gtvfx-contrib/team" -Repos @("repo1", "repo2") -Path "root"
```
...
**Parameters:**
- `Owner` - Code owner to search for (required, e.g., '@gtvfx-contrib/team' or '@username')
- `Org` - Organization name (default: 'gtvfx-contrib')
- `Repos` - Array of repository names (optional)
...
$repos = Find-GitHubReposByCodeowner -Owner "@gtvfx-contrib/envoy-internal-dev"

# Detailed output with matching lines
Find-GitHubReposByCodeowner -Owner "@gtvfx-contrib/art_team" -OutputFormat "detailed"

# Search with skip patterns
Find-GitHubReposByCodeowner -Owner "@username" -SkipPattern @("gt-ext-*", "*-archive")

# JSON output
Find-GitHubReposByCodeowner -Owner "@gtvfx-contrib/team" -OutputFormat "json"
```
...
```powershell
Import-Module "V:\repo\gt\devtools\pwsh\github_cli\GitHubTeamManagement.psm1"

# Get repos excluding external repositories
$repos = Get-GitHubRepos -SkipPattern "gt-ext-*"

# Add team to filtered repos
Add-GitHubTeamToRepos -Team "envoy-internal-dev" -Permission "push" -Repos $repos.name
```
...
$content = @(
    ...
    "",
    "# Documentation",
    "*.md @gtvfx-contrib/tech_writers"
)

# Update all repos except archived and external ones
Update-GitHubCodeowners -Content $content -SkipPattern @("*-archive", "gt-ext-*", "*-old")
```
...
$results = Find-GitHubReposByCodeowner -Owner "@gtvfx-contrib/envoy-internal-dev" -OutputFormat "detailed"
...
```powershell
Import-Module "Z:\repo\gt\command_aliases\ps\github_cli\GitHubTeamManagement.psm1"
...
$repos = Find-GitHubReposByCodeowner -Owner "@gtvfx-contrib/old_team"
...
gh api /orgs/gtvfx-contrib/teams

# List repositories
gh repo list gtvfx-contrib
...
- Ensure you're using the correct hostname: `<GitHub URL>`
- Check your SSH key is configured: `ssh -T git@<GitHub URL>`
...
- Check team membership: `gh api /orgs/gtvfx-contrib/teams/[team-name]/members`
